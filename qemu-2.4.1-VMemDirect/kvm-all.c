...
#ifdef TAKAHIRO
static void print_histogram(unsigned char *history, unsigned long nr_pages);
...
#define KVM_GET_surote_STATE _IOR(KVMIO, 0xb8, struct kvm_surote_state)

struct kvm_surote_state
{
    unsigned long *bitmap;
    unsigned long nr_pages;//追加
};

#define PAGE_SIZE_2 4096
#define PAGE_SHIFT 12
#define VM_MEM (14ULL << 30)
...
int kvm_cpu_exec(CPUState *cpu)
{
    ...
    do {
        ...
        if (current_time.tv_sec - last_time.tv_sec >= 1 && cpu->cpu_index == 0) {
            struct kvm_surote_state data;
            //unsigned long nr_pages = DIV_ROUND_UP(VM_MEM, PAGE_SIZE_2);
            //printf("nr_pages1 = %lu\n", nr_pages);
            //unsigned long nr_pages = DIV_ROUND_UP(last_ram_offset(), PAGE_SIZE_2);
            //printf("nr_pages2 = %lu\n", nr_pages);
            unsigned long mem_size = get_main_mem_size();
           unsigned long nr_pages = mem_size / 4096;
            unsigned long size = DIV_ROUND_UP(nr_pages, BITS_PER_BYTE);
            // fprintf(stderr, "1sec\n");
            // unsigned long mem_size = get_main_mem_size();
            // fprintf(stderr, "After get_main_mem_size\n");
            unsigned long pfn;
            unsigned long count_bit = 0;
            // unsigned long nr_pages = mem_size / 4096;
            // unsigned long size = nr_pages / 8;
            int a3;            

            count ++;

            data.bitmap = g_malloc(size);
            memset(data.bitmap, 0, size);

            data.nr_pages = nr_pages;

            a1= qemu_clock_get_ns(QEMU_CLOCK_HOST);

//            fprintf(stderr, "Before EPT : %ld\n", qemu_clock_get_ns(QEMU_CLOCK_HOST));

            kvm_vcpu_ioctl(cpu, KVM_GET_surote_STATE, &data);

            a2=qemu_clock_get_ns(QEMU_CLOCK_HOST);

//            fprintf(stderr, "EPT collected time : %ld\n", qemu_clock_get_ns(QEMU_CLOCK_HOST));
             a3 = a2-a1;
	     ...
	     //             printf("EPTtime= %d\n",a3);
            ...
            for (pfn = 0; pfn < nr_pages; pfn++) {
                if (test_bit(pfn, data.bitmap)){
                    ...
                    if(count == 2){
                      ...
                      history[pfn] = 1 << 7 | history[pfn] >> 1;
                        //fprintf(stderr, "history[%lx] : %x\n", pfn, history[pfn]);
                    } else {
                      history[pfn] = 1 << 7 | history[pfn];
                    }
#endif
                }
#ifdef LRU8
            ...
            print_histogram(history, nr_pages);
            if(count == 2){
              count = 0;
              // fprintf(stderr, "shift bit\n" );
            }
...
int get_mem_usage(unsigned long **bitmap)
{
    struct kvm_surote_state data;
    unsigned long nr_pages = DIV_ROUND_UP(last_ram_offset(), TARGET_PAGE_SIZE);
    unsigned long size = DIV_ROUND_UP(nr_pages, BITS_PER_BYTE);

    fprintf(stderr, "IN_get_mem_usage\n");
    data.bitmap = g_malloc(size);
    fprintf(stderr, "g_malloc\n");
    memset(data.bitmap, 0, size);
    fprintf(stderr, "memset\n");

    kvm_vcpu_ioctl(current_cpu, KVM_GET_surote_STATE, &data);
